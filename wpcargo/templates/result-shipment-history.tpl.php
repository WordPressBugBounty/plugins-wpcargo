...
<div id="wpcargo-history-section" class="wpcargo-history-details print-section wpcargo-table-responsive table-responsive">
	<p class="header-title"><strong><?php echo esc_html(apply_filters( 'wpc_shipment_history_header', __( 'Shipment History' , 'wpcargo') )); ?></strong></p>
	...
	<table id="shipment-history" class="table wpcargo-table" style="width: 100%;">
        <thead>
			...
			<?php foreach( wpcargo_history_fields() as $history_name => $history_fields ): ?>
				<th><?php echo esc_html($history_fields['label']); ?></th>
			<?php endforeach; ?>
                ...
                foreach($sort_shipment_history as $shipments){
                    ...
                    <tr class="history-row">
						<?php foreach( wpcargo_history_fields() as $history_name => $history_fields ): ?>
							...
							?>
							<td class="history-data <?php echo esc_html(wpcargo_to_slug($history_name)); ?> <?php echo esc_html(wpcargo_to_slug($value)); ?>"><?php echo esc_html( $value ); ?></td>
						<?php endforeach; ?>
